...
async def cmd_newgame(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Agar game chal raha hai (waiting ya active) to block karo
    if chat_id in GAMES:
        existing = GAMES[chat_id]
        if existing.phase == Phase.WAITING:
            # Purana join message resend karo
            try:
...
"
                    f"👥 Players: {count}  {chr(40)}min 4 needed{chr(41)}

"
                    f"{plines if plines else chr(40)+chr(41)}

"
                    f"_Join karo ya start karo!_",
                    reply_markup=InlineKeyboardMarkup(kb),
                    parse_mode="Markdown"
                )
            except Exception:
                pass
            return
        elif existing.phase not in [Phase.FINISHED]:
