...
async def join_button(bot, chat_id):
    """Build join button with deep link to private chat"""
    me = await bot.get_me()
    return InlineKeyboardButton(
        "✅  Join to the game",
        url=f"https://t.me/{me.username}?start=join_{chat_id}"
    )

async def start_button(chat_id):
    return InlineKeyboardButton("🚀  Start the game!", callback_data=f"sg_{chat_id}")
...
class Game:
    def __init__(self, chat_id):
        ...
        self.vote_live_msg_id = None  # Live vote update message
...
async def auto_cancel_game(chat_id, ctx):
    """10 minute mein game auto reset - agar start nahi hua"""
    ...
    if game and game.phase == Phase.WAITING:
        ...
        try:
            await ctx.bot.send_message(
                chat_id,
                "⏰ **10 minute ho gaye — game cancel ho gaya!**\n\nNaya game shuru karne ke liye /newgame likho.",
                parse_mode="Markdown"
            )
        except Exception:
            pass
...
async def cmd_start_private(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    args = ctx.args  # e.g. ["join_-100123456"]

    # Deep link join: /start join_CHATID
    if args and args[0].startswith("join_"):
        try:
            group_chat_id = int(args[0].split("_")[1])
            game = get_game(group_chat_id)
            if not game or game.phase != Phase.WAITING:
                await update.message.reply_text("⚠️ Woh game ab available nahi hai!")
                return
            if user.id in game.players:
                await update.message.reply_text("✅ Tum pehle se join ho! Group mein wapas jao.")
                return
            if len(game.players) >= 15:
                await update.message.reply_text("⚠️ Game full hai!")
                return
            ...
            # Update group join message
            plines = "\n".join(f"  {i+1}. {pl.display}" for i, pl in enumerate(game.players.values()))
            me = await ctx.bot.get_me()
            kb = [[InlineKeyboardButton("✅  Join to the game", url=f"https://t.me/{me.username}?start=join_{group_chat_id}")]]
            if count >= 4:
                kb.append([InlineKeyboardButton("🚀  Start the game!", callback_data=f"sg_{group_chat_id}")])
            try:
            ...
            await update.message.reply_text(
                f"✅ **Tum join ho gaye!**\n\nGroup mein wapas jao game khelne ke liye. 🎭",
                parse_mode="Markdown"
            )

            # Reset 10 min timer
            if hasattr(game, '_timeout_task') and game._timeout_task:
                game._timeout_task.cancel()
            game._timeout_task = asyncio.create_task(auto_cancel_game(group_chat_id, ctx))
            return
        except Exception as e:
            await update.message.reply_text(f"⚠️ Kuch gadbad hui: {e}")
            return

    me = await ctx.bot.get_me()
    kb = [
        [InlineKeyboardButton("➕ Add game to your chat", url=f"https://t.me/{me.username}?startgroup=go")],
...
async def cmd_newgame(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in GAMES:
        ...
        if existing.phase == Phase.WAITING:
            count = len(existing.players)
            plines = "\n".join(f"  {i+1}. {p.display}" for i, p in enumerate(existing.players.values()))
            me = await ctx.bot.get_me()
            kb = [[InlineKeyboardButton("✅  Join to the game", url=f"https://t.me/{me.username}?start=join_{chat_id}")]]
            if count >= 4:
                kb.append([InlineKeyboardButton("🚀  Start the game!", callback_data=f"sg_{chat_id}")])
            await update.message.reply_text(
                f"🎭 **Game already recruiting!**\n\n"
                f"👥 Players: {count} (min 4 needed)\n\n"
                f"{plines if plines else '(koi nahi)'}\n\n"
                f"_Join button dabao ya start karo!_",
                reply_markup=InlineKeyboardMarkup(kb),
                parse_mode="Markdown"
            )
            return
        elif existing.phase != Phase.FINISHED:
            count = len(existing.alive_players)
            await update.message.reply_text(
                f"⚠️ **Game abhi chal raha hai!**\n\n"
                f"Zinda khiladi: {count}\n"
                f"Jab game khatam ho tab /newgame karo.",
                parse_mode="Markdown"
            )
            return
    ...
    msg = await update.message.reply_text(
        ...
        "👥 Players: 0 (min 4 needed)\n\n"
        "Join button dabao — private chat mein jaoge aur auto join ho jaoge!",
        reply_markup=InlineKeyboardMarkup(kb),
        parse_mode="Markdown"
    )
    game.join_msg_id = msg.message_id

    # 10 min auto cancel timer shuru karo
    game._timeout_task = asyncio.create_task(auto_cancel_game(chat_id, ctx))
...
async def cmd_join(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    game    = get_game(chat_id)
    if not game or game.phase != Phase.WAITING:
        await update.message.reply_text("⚠️ Koi active game nahi! Pehle /newgame karo.")
        return
    me = await ctx.bot.get_me()
    kb = [[InlineKeyboardButton("✅  Join to the game", url=f"https://t.me/{me.username}?start=join_{chat_id}")]]
    await update.message.reply_text(
        "👆 Button dabao — bot private chat mein le jayega aur tum auto join ho jaoge!",
        reply_markup=InlineKeyboardMarkup(kb)
    )
...
async def update_live_votes(chat_id, ctx):
    """Group mein live vote board update karo"""
    game = get_game(chat_id)
    if not game:
        return

    # Build vote board: kisne kisko vote diya
    lines = []
    for voter_id, target_str in game.day_votes.items():
        voter = game.players.get(voter_id)
        if not voter:
            continue
        if target_str == "skip":
            lines.append(f"  {voter.display} → ⏩ Skip")
        else:
            try:
                target = game.players.get(int(target_str))
                if target:
                    weight = voter.vote_weight
                    w_str = " (x2 👑)" if weight == 2 else ""
                    lines.append(f"  {voter.display} → 🗳 {target.display}{w_str}")
            except Exception:
                pass

    not_voted = [p for p in game.alive_players if p.user_id not in game.day_votes]
    not_voted_str = ", ".join(p.display for p in not_voted) if not_voted else "Sab ne vote kiya! ✅"

    text = (
        f"🗳 **Live Votes — Day {game.day}**\n\n"
        f"{chr(10).join(lines) if lines else '_(abhi kisi ne vote nahi kiya)_'}\n\n"
        f"⏳ **Abhi tak nahi vote kiya:**\n  {not_voted_str}"
    )

    try:
        if game.vote_live_msg_id:
            await ctx.bot.edit_message_text(
                chat_id=chat_id,
                message_id=game.vote_live_msg_id,
                text=text,
                parse_mode="Markdown"
            )
        else:
            msg = await ctx.bot.send_message(chat_id, text, parse_mode="Markdown")
            game.vote_live_msg_id = msg.message_id
    except Exception:
        pass
...
async def cb_day_vote(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Live vote board update karo
    await update_live_votes(chat_id, ctx)
